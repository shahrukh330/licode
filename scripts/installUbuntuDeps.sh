...
install_apt_deps(){
  ...
  sudo apt-get --assume-yes install -qq python-software-properties -y
  sudo apt-get --assume-yes install -qq software-properties-common -y
  sudo add-apt-repository ppa:ubuntu-toolchain-r/test -y
  sudo apt-get --assume-yes update -y
  sudo apt-get --assume-yes install -qq git make gcc-5 g++-5 libssl-dev cmake libglib2.0-dev pkg-config libboost-regex-dev libboost-thread-dev libboost-system-dev liblog4cxx10-dev rabbitmq-server mongodb openjdk-6-jre curl libboost-test-dev -y
...
install_libnice(){
  if [ -d $LIB_DIR ]; then
    ...
    if [ ! -f ./libnice-0.1.7.tar.gz ]; then
      curl -OL https://nice.freedesktop.org/releases/libnice-0.1.7.tar.gz
